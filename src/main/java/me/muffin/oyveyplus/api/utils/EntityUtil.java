...
import me.muffin.oyveyplus.impl.mixin.mixins.IEntityLivingBase;
...
public class EntityUtil implements Wrapper {
    ...
    public static boolean isPlayerValid(final EntityPlayer player, final float range) {
        return player != mc.player && mc.player.getDistance(player) < range && !isDead(player) && !OyVeyPlus.friendManager.isFriend(player.getName());
    }
    ...
    public static Vec3d getInterpolatedRenderPos(Entity entity, float partialTicks) {
        return EntityUtil.getInterpolatedPos(entity, partialTicks).subtract(EntityUtil.mc.getRenderManager().renderViewEntity.posX, EntityUtil.mc.getRenderManager().renderViewEntity.posY, EntityUtil.mc.getRenderManager().renderViewEntity.posZ);
    }

    public static Vec3d getInterpolatedRenderPos(Vec3d vec) {
        return new Vec3d(vec.x, vec.y, vec.z).subtract(EntityUtil.mc.getRenderManager().renderViewEntity.posX, EntityUtil.mc.getRenderManager().renderViewEntity.posY, EntityUtil.mc.getRenderManager().renderViewEntity.posZ);
    }
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && OyVeyPlus.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends) {
