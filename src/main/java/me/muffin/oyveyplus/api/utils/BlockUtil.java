...
public class BlockUtil implements Wrapper {
    ...
    public static boolean placeBlock(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        for(BlockPos b : BlockUtil.getFriends(pos))
        {
            if(usable==false)
            {
                for(BlockPos b1 : BlockUtil.getFriends(b))
                {
                    if(mc.world.getBlockState(b1).getBlock()!= Blocks.AIR)
                    {
                        usable=true;
                        neighbour=b;
                    }
        ...
        if(rotate)
        {
            RotationUtil.rotate(neighbour, true);
        }
        BlockUtil.mc.player.swingArm(EnumHand.MAIN_HAND);
        BlockUtil.rightClickBlock(pos, hitVec, hand, opposite, packet);
        if(rotate)
        {
            RotationUtil.rotate(pos, true);
        }
        BlockUtil.mc.player.swingArm(EnumHand.MAIN_HAND);;
