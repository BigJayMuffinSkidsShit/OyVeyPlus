...
public class Surround
    ...
    private final Setting<Double> blocksPerTick = this.register("BlocksPerTick", 12, 1, 20,0);
    private final Setting<Double> delay = this.register("Delay", 0, 0, 250,0);
    private final Setting<Boolean> noGhost = this.register("PacketPlace", false);
    private final Setting<Boolean> center = this.register("TPCenter", false);
    private final Setting<Boolean> rotate = this.register("Rotate", true);
    private final Setting<Boolean> antipedo = this.register("AntiPedo",true);
    private final Setting<Boolean> floor = this.register("Floor",false);
    private final Setting<Boolean> helpingBlocks = this.register("HelpBlocks",false);
    ...
    private void doFeetPlace() {
        ...
        if (!BlockUtil.isSafe(mc.player, onEChest ? 1:0, this.floor.getValue())) {
    ...
    private boolean check() {
        ...
        if (obbySlot == -1 && !offHand && echestSlot == -1) {
            MessageUtil.instance.addMessage("<Surround> " + ChatFormatting.RED + "No Obsidian in hotbar disabling...",false);
            disable();
            return true;
        }
        isSneaking = EntityUtil.stopSneaking(isSneaking);
