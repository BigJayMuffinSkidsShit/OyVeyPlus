...
public class AutoTrap extends Module {
    ...
    public AutoTrap() {
        super("Trap", "Traps enemiez", Category.Combat);
    }
    public Setting<Double> delay = this.register("Delay",50,0,250,0);
    public Setting<Double> blocksPerPlace = this.register("BPT",18,0,20,1);
    public Setting<Boolean> rotate = this.register("Rotations",false);
    public Setting<Boolean> raytrace = this.register("Raytrace",true);
    public Setting<Boolean> antiScaffold = this.register("AntiScaffold",true);
    ...
    private boolean check() {
        ...
        if (obbySlot == -1) {
            MessageUtil.instance.addMessage("<" + "Trap" + "> " + ChatFormatting.RED + "No Obsidian in hotbar disabling...", false);
            disable();
            return true;
        }
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        for (EntityPlayer player : AutoTrap.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, range) || trapped && EntityUtil.isTrapped(player, antiScaffold.getValue(), antiStep.getValue(), false, false, false))
                continue;
            if (target == null) {
                target = player;
